...
d.prototype.generateRequestId_=function(){return(new Date).getTime()+Math.random()};d.prototype.cookieProvider_=null;d.prototype.subscriptions_=null;d.prototype.socketNamespace_=null;d.prototype.connect_=function(){var a=this;a.ajax_.requestJson(a.masterUrl+"/getWorker",function(e,c){e||!c||!c.host||!c.port?setTimeout(function(){a.connect_()},5E3):(a.socketNamespace_=io.connect((c.secure?"https":"http")+"://"+c.host+":"+c.port,{resource:"viewhub",reconnect:!1}),a.socketNamespace_.on("newValue",function(c,
e,b,d){a.newValueHandler_(c,e,b,d)}),a.socketNamespace_.on("invokeMethodResponse",function(c){a.responseHandler_(c)}),a.socketNamespace_.on("subscribeResponse",function(c){a.responseHandler_(c)}),a.socketNamespace_.on("unsubscribeResponse",function(c){a.responseHandler_(c)}),a.socketNamespace_.on("editResponse",function(c){a.responseHandler_(c)}),a.socketNamespace_.on("getModelResponse",function(c){a.responseHandler_(c)}),a.socketNamespace_.on("connect",function(){a.connectHandler_();a.emit("connect",
c)}),a.socketNamespace_.on("disconnect",function(){a.disconnectHandler_();a.emit("disconnect")}),a.emit("socketReady"))})};d.prototype.invoke=function(a,e,c,b){var g=this,d=g.generateRequestId_(),f=Array.prototype.slice.call(arguments,3),j=function(){!a||!e?c&&c(Error("View name and method name must be specified")):(g.once("response"+d,function(a){g.removeAllListeners("response"+d);a.success?c&&c(null,a.result):c&&c(Error("Server declined method invocation"))}),g.socketNamespace_.emit("invokeMethodRequest",
{viewName:a,methodName:e,args:f,sessionId:g.sessionId,id:d}))};if(g.isSocketReady_)j();else g.once("socketReady",j)};d.prototype.getModel=function(a,e){var c=this,b=c.generateRequestId_(),d=function(){a?(c.once("response"+b,function(a){c.removeAllListeners("response"+b);a.success?e&&e(null,a.model):e&&e(Error("Server declined model request"))}),c.socketNamespace_.emit("getModelRequest",{viewName:a,sessionId:c.sessionId,id:b})):e&&e(Error("View name must be specified"))};if(c.isSocketReady_)d();else c.once("socketReady",
d)};d.prototype.setValue=function(a,e,c,b){var d=this,f=d.generateRequestId_(),k=function(){!a||!e?b&&b(Error("View name and property name must be specified")):(d.once("response"+f,function(a){d.removeAllListeners("response"+f);a.success?b&&b(null):b&&b(Error("Server declined new value of property"))}),d.socketNamespace_.emit("editRequest",{viewName:a,propertyName:e,newValue:c,sessionId:d.sessionId,id:f}))};if(d.isSocketReady_)k();else d.once("socketReady",k)};d.prototype.subscribe=function(a,e,c){if(!a||
!e)c&&c(Error("View name and handler must be specified"));else{var b=this,d=b.generateRequestId_(),f=function(){b.once("response"+d,function(f){b.removeAllListeners("response"+d);f.success?(b.subscriptions_[a]||(b.subscriptions_[a]=[]),b.subscriptions_[a].push(e),c&&c(null)):c&&c(Error("Server declined subscription"))});b.socketNamespace_.emit("subscribeRequest",{viewName:a,sessionId:b.sessionId,id:d})};if(b.isSocketReady_)f();else b.once("socketReady",f)}};d.prototype.unsubscribe=function(a,b,c){if(!a||
!b)c&&c(Error("View name and handler must be specified"));else if(!d.subscriptions_[a]||0===d.subscriptions_[a].length)c&&c(Error("This view has no subscriptions yet"));else{var d=this,f=d.generateRequestId_(),h=function(){d.once("response"+f,function(h){d.removeAllListeners("response"+f);var j=d.subscriptions_[a].indexOf(b);d.subscriptions_.splice(j,1);h.success?c&&c(null):c&&c(Error("Server declined unsubscribe message"))});d.socketNamespace_.emit("unsubscribeRequest",{viewName:a,sessionId:d.sessionId,
id:f})};if(d.isSocketReady_)h();else d.once("socketReady",h)}};d.prototype.disconnectHandler_=function(){this.isSocketReady_=!1;this.socketNamespace_.removeAllListeners();this.connect_()};d.prototype.connectHandler_=function(){for(var a in this.subscriptions_)if(this.subscriptions_.hasOwnProperty(a)){var b=this.generateRequestId_();this.socketNamespace_.emit("subscribeRequest",{viewName:a,sessionId:this.sessionId,id:b})}};d.prototype.responseHandler_=function(a){a.requestId&&a.sessionId&&(this.sessionId=
a.sessionId,this.cookieProvider_.setCookie("sid",a.sessionId),this.emit("response"+a.requestId,a.responseBody))};d.prototype.newValueHandler_=function(a,b,c,d){this.subscriptions_.hasOwnProperty(a)&&this.subscriptions_[a].forEach(function(a){a(b,c,d)})};f.prototype.setCookie=function(a,b,c,d,f,h){if(!a||!b)return!1;a=a+"="+encodeURIComponent(b);c&&(a+="; expires="+c.toGMTString());d&&(a+="; path="+d);f&&(a+="; domain="+f);h&&(a+="; secure");document.cookie=a;return!0};f.prototype.getCookie=function(a){return RegExp("(?:; )?"+
a+"=([^;]*);?").test(document.cookie)?decodeURIComponent(RegExp.$1):null};f.prototype.removeCookie=function(a,b,c){this.setCookie(a,null,new Date(0),b,c);return!0};b.prototype.createRequest_=function(){if("undefined"!==typeof XMLHttpRequest)return new XMLHttpRequest;if("undefined"!==typeof ActiveXObject)try{return new ActiveXObject("Msxml2.XMLHTTP")}catch(a){return new ActiveXObject("Microsoft.XMLHTTP")}throw Error("AJAX is not supported");};b.prototype.request=function(a,b){var c=this.createRequest_();
c.open("GET",a,!0);c.send(null);if(b){var d=function(){4!=c.readyState?setTimeout(d,10):200==c.status?b(null,c.responseText):b(Error(c.status),null)};setTimeout(d,10)}};b.prototype.requestJson=function(a,b){this.request(a,function(a,d){try{var f=(new Function("return "+d))();b(null,f)}catch(h){b(h,null)}})}})();